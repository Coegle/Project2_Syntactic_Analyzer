...
// 求非终结符的first集
void LALR_Parser::getFirstSetofNonterminal() {
	int change = 0;
...
// 判断是否是终结符
bool LALR_Parser::isTerminal(string& s)
{
...
// 求string的first集
set<string> LALR_Parser::getFirstSetofStrings(vector<string>& strings)
{
	set<string> firstSet;
...
void LALR_Parser::creatTable()
{
	...
	while (q.size())
	{
		...
		for (int j = 0; j < terminal.size(); j++) { // 对于所有终结符
			LALRItems nextItms = getNextItems(itemsList[i], terminal[j]);
		...
		for (auto it_nontml = firstofNonterminal.begin(); it_nontml != firstofNonterminal.end(); it_nontml++) { // 对所有非终结符
			LALRItems nextItms = getNextItems(itemsList[i], it_nontml->first);
			if (nextItms.items.size() == 0) continue;
			auto hasExisted = find(itemsList.begin(), itemsList.end(), nextItms); // 是否已经存在
			if (hasExisted == itemsList.end()) {
				itemsList.push_back(nextItms);
...
void LALR_Parser::parse(const char* tokenListPath) 
{
	ifstream fin;
...
// 求下一个项目集
LALRItems LALR_Parser::getNextItems(LALRItems& preItems, string token)
{
	LALRItems nextItms;
...
// 求项目集的闭包
void LALR_Parser::getClousureofItems(LALRItems& items)
{
	int change;
	do {
		change = 0;
...
void LALR_Parser::inputBNF(const char* BNFPath)
{
	ifstream fin;
	fin.open(BNFPath);
	if (fin.fail()) {
...
// 从文件读入action和goto表
void LALR_Parser::inputTable(const char* gotoListPath, const char* actionListPath)
{
...
// 输出action和goto表
void LALR_Parser::outputTable(const char* gotoListPath, const char* actionListPath)
{
...
vector<string> LALR_Parser::split(string str, string pattern)
{
